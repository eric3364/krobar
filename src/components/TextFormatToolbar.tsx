...
export default function TextFormatToolbar({ rect, value, onChange, selectionCount = 1 }: Props) {
  ...
  return (
    <div
      ...
      style={{ left, top }}
      onPointerDown={(e) => { const tag = (e.target as HTMLElement).tagName; if (tag !== "SELECT" && tag !== "OPTION") e.preventDefault(); e.stopPropagation(); }}
      onMouseDown={(e) => { const tag = (e.target as HTMLElement).tagName; if (tag !== "SELECT" && tag !== "OPTION") e.preventDefault(); e.stopPropagation(); }}
